...
class Node:
    def __init__(self, tag = None, content = b'', line_count = 0, opener = b'', op_lines = 0, closer = b'', cl_lines = 0):
        self._tag = tag
        self._opener = opener
        self._op_lines = op_lines
        self._closer = closer
        self._cl_lines = cl_lines
    ...
    def create_new_branch(self, tag = None, opener = b'', op_lines = 0):
        ...
        child = Node(tag, opener = opener, op_lines = op_lines)
        self.children.append(child)
        return child
        
    def seal(self, closer = b'', cl_lines = 0):
        #print(">seal()")
        if self.children:
            last = self.children[-1]
            if last._content is None or len(last._content) == 0:
                self.children.pop()
        assert len(self._closer) == 0 and self._cl_lines == 0
        self._closer = closer
        self._cl_lines = cl_lines
        self._sealed = True
    ...
    def byte_size(self):
        return (len(self._opener) 
          + (len(self._content) if not self.children else sum([_.byte_size() for _ in self.children]))
          + len(self._closer))

    def line_count(self):
        return (self._op_lines
          + (self._line_count if not self.children else sum([_.line_count() for _ in self.children])) 
          + self._cl_lines)
        ...
        for line_buf in stream:
            ...
            if line[:2] == '#$': # TODO: support other comment introducers
                if line[2] == '[':
                    ...
                    tag = line[3:].strip()
                    child = curr_branch[-1].create_new_branch(tag, line_buf, 1)
                    curr_branch.append(child)
                elif line[2] == ']': # ignoring the rest of the line (for now)
                    curr_branch[-1].seal(line_buf, 1)
                    curr_branch.pop()
                    if indent != indents[-1]:
